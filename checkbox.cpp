...
void Checkbox::draw( ) {
	...
	if( m_use_label )
	    render::menu_shade.string( p.x + LABEL_OFFSET, p.y - 3, { 255, 255, 255, m_parent->m_alpha }, m_label );

	// render border.
	render::rect( p.x + 1, p.y + 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, { 255, 255, 255, m_parent->m_alpha } );

	// render checked.
	if( m_checked ) {
		render::rect_filled( p.x + 1, p.y + 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, color );
		render::rect_filled_fade( p.x + 1, p.y + 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, { 255, 255, 255, m_parent->m_alpha }, 0, 150 );
	}

	else
		render::gradient( p.x + 1, p.y + 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, { 245, 245, 245, m_parent->m_alpha }, { 245, 255, 245, m_parent->m_alpha } );
