...
static float s_anim_offset =
    0.f; // 0..1, cycles continuously for gradient + glow
...
static Color lerp_color(Color a, Color b, float t) {
  ...
  return Color{(int)a.r() + (int)(((int)b.r() - (int)a.r()) * t),
...
void renderWatermark(int screen_width, int screen_height) {
  ...
  const std::string ping_str =
      cfg.show_ping ? tfm::format(XOR("ping: %i"), s_ping) : "";
  const std::string tick_str =
      cfg.show_tick ? tfm::format(XOR("tick: %i"), s_tick) : "";
  const std::string user_str =
      (cfg.show_user && !g_cl.m_user.empty()) ? g_cl.m_user : "";
  ...
  const auto fps_sz =
      cfg.show_fps ? s_stat_font.size(fps_str) : render::FontSize_t{};
  const auto ping_sz =
      cfg.show_ping ? s_stat_font.size(ping_str) : render::FontSize_t{};
  const auto tick_sz =
      cfg.show_tick ? s_stat_font.size(tick_str) : render::FontSize_t{};
  const auto time_sz =
      cfg.show_time ? s_stat_font.size(s_time_str) : render::FontSize_t{};
  const auto user_sz = (cfg.show_user && !user_str.empty())
                           ? s_stat_font.size(user_str)
                           : render::FontSize_t{};

  // --- Total panel width ---
  int content_w = pad_x + title_sz.m_width;
  if (cfg.show_fps)
    content_w += sep_sz.m_width + fps_sz.m_width;
  if (cfg.show_ping)
    content_w += sep_sz.m_width + ping_sz.m_width;
  if (cfg.show_tick)
    content_w += sep_sz.m_width + tick_sz.m_width;
  if (cfg.show_time)
    content_w += sep_sz.m_width + time_sz.m_width;
  if (cfg.show_user && !user_str.empty())
    content_w += sep_sz.m_width + user_sz.m_width;
  content_w += pad_x;
  ...
  if (cfg.debug_mode) {
    const float frametime_ms =
        g_csgo.m_globals ? g_csgo.m_globals->m_frametime * 1000.f : 0.f;
    const std::string dbg =
        tfm::format(XOR("build: beta | ft: %.2fms"), frametime_ms);
    const auto dbg_sz = s_stat_font.size(dbg);
    const int dbg_h = dbg_sz.m_height + pad_y;
    const int dbg_y = panel_y + panel_h + 1;
    render::rect_filled(panel_x, dbg_y, content_w, dbg_h, {8, 8, 14, alpha_bg});
    s_stat_font.string(panel_x + pad_x, dbg_y + pad_y / 2,
