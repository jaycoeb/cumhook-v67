...
void Form::draw( ) {
	...
	// background.
	render::rect_filled( m_x, m_y, m_width, m_height, { 255, 255, 255, m_alpha } );

	// border.
	render::rect( m_x, m_y, m_width, m_height, { 5, 5, 5, m_alpha } );
	render::rect( m_x + 1, m_y + 1, m_width - 2, m_height - 2, { 255, 255, 255, m_alpha } );
	render::rect( m_x + 2, m_y + 2, m_width - 4, m_height - 4, { 255, 255, 255, m_alpha } );
	render::rect( m_x + 3, m_y + 3, m_width - 6, m_height - 6, { 255, 255, 255, m_alpha } );
	render::rect( m_x + 4, m_y + 4, m_width - 8, m_height - 8, { 255, 255, 255, m_alpha } );
	render::rect( m_x + 5, m_y + 5, m_width - 10, m_height - 10, { 255, 255, 255, m_alpha } );
	...
	if( !m_tabs.empty( ) ) {
		...
		render::rect_filled( tabs_area.x, tabs_area.y, tabs_area.w, tabs_area.h, { 255, 255, 255, m_alpha } );
		render::rect( tabs_area.x, tabs_area.y, tabs_area.w, tabs_area.h, { 255, 255, 255, m_alpha } );
		render::rect( tabs_area.x + 1, tabs_area.y + 1, tabs_area.w - 2, tabs_area.h - 2, { 255, 255, 255, m_alpha } );
		...
		if( !m_active_tab->m_elements.empty( ) ) {
			...
			render::rect( el.x + 1, el.y + 1, el.w - 2, el.h - 2, { 255, 255, 255, m_alpha } );
