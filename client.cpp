...
void Client::KillFeed() {
  ...
  KillFeed_t *feed = (KillFeed_t *)g_csgo.m_hud->FindElement(
      HASH("SFHudDeathNoticeAndBotStatus"));
  if (!feed)
    return;
...
void Client::OnMapload() {
  ...
  m_local = g_csgo.m_entlist->GetClientEntity<Player *>(
  ...
  if (g_csgo.m_net) {
    ...
    g_hooks.m_net_channel.add(INetChannel::PROCESSPACKET,
                              util::force_cast(&Hooks::ProcessPacket));
    g_hooks.m_net_channel.add(INetChannel::SENDDATAGRAM,
                              util::force_cast(&Hooks::SendDatagram));
  }
}
...
void Client::StartMove(CUserCmd *cmd) {
  ...
  // because prediction runs on frames, when we have low fps it might not
  // predict all usercommands. also fix the tick being inaccurate.
  g_inputpred.update();
...
void Client::DoMove() {
  ...
  if (m_weapon) {
    ...
    // run autowall once for penetration crosshair if we have an appropriate
    // weapon.
    if (m_weapon_type != WEAPONTYPE_KNIFE && m_weapon_type != WEAPONTYPE_C4 &&
        m_weapon_type != WEAPONTYPE_GRENADE) {
    ...
    // can the player fire.
    m_player_fire = g_csgo.m_globals->m_curtime >= m_local->m_flNextAttack() &&
  ...
  //              add indicator if valid (on ground, still have time, not being
  //              defused already, etc). move this? not sure where we should put
  //              it.
  if (g_input.GetKeyState(g_menu.main.misc.last_tick_defuse.get()) &&
      g_visuals.m_c4_planted) {
    float defuse = (m_local->m_bHasDefuser()) ? 5.f : 10.f;
    float remaining =
        g_visuals.m_planted_c4_explode_time - g_csgo.m_globals->m_curtime;
...
void Client::SetAngles2(ang_t angle) {
  if (!g_cl.m_local || !g_cl.m_processing ||
      !g_menu.main.antiaim.fake_yaw.get())
    return;
...
void Client::print(const std::string text, ...) {
  ...
  // print to console.
  g_csgo.m_cvar->ConsoleColorPrintf(colors::burgundy, XOR("[Cumhook] "));
...
void Client::UpdateRevolverCock() {
  ...
  if (m_weapon_id != REVOLVER || m_weapon->m_iClip1() < 1 || !m_player_fire ||
      g_csgo.m_globals->m_curtime < m_weapon->m_flNextPrimaryAttack()) {
    // reset.
    m_revolver_cock = 0;
    m_revolver_query = 0;
    return;
  ...
  int shoot =
      (int)(0.25f /
            (std::round(g_csgo.m_globals->m_interval * 1000000.f) / 1000000.f));
  ...
  if (m_revolver_query == m_revolver_cock) {
    ...
    // apply inattack.
    if (g_menu.main.config.mode.get() == 0 &&
        m_revolver_query > m_revolver_cock)
...
void Client::UpdateIncomingSequences() {
  ...
  if (m_sequences.empty() ||
      g_csgo.m_net->m_in_seq > m_sequences.front().m_seq) {
    // store new stuff.
    m_sequences.emplace_front(g_csgo.m_globals->m_realtime,
                              g_csgo.m_net->m_in_rel_state,
                              g_csgo.m_net->m_in_seq);
  }
